...
plt.plot(np.arange(len(AVG_REWARD_BUFFER)),
         gaussian_filter1d(AVG_REWARD_BUFFER, sigma=11),
...
if IS_TEST:
    ...
    for episode_i in range(NUM_EPISODE_TEST):
        ...
        while not done:
            ...
            action = np.argmax(agent.q_table1[state_key])
    ...
    print("\nTest Results:")
    print(f"Total episodes: {NUM_EPISODE_TEST}")
    print(f"Avg. reward: {total_rewards/NUM_EPISODE_TEST:.3f}")
